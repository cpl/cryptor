...
func Initialize(iSPub, rSPub ppk.PublicKey) (
	hs *Handshake, encISPub [sizeEncPub]byte) {
	// create handshake
	hs = new(Handshake)
	...
	cipher.Seal(encISPub[:0], zeroNonce[:], iSPub[:], hs.hash[:])
...
func Respond(iTPub ppk.PublicKey, rSec ppk.PrivateKey, encISPub [sizeEncPub]byte) (
	hs *Handshake, iSPub ppk.PublicKey, encNothing [sizeEncNth]byte, err error) {
	...
	// derive chaining key
	hkdf.HKDF(hs.hash[:], iTPub[:], &hs.c)

	// compute shared secret
	ss := rSec.SharedSecret(iTPub)
	...
	if err != nil {
		return nil, iSPub, encNothing, err
	}
	_, err = cipher.Open(iSPub[:0], zeroNonce[:], encISPub[:], hs.hash[:])
	if err != nil {
		return nil, iSPub, encNothing, err
	}
	...
	ss = hs.tempKeys.secret.SharedSecret(iTPub)
	...
	// encrypt nothing
	cipher, _ = chacha.New(hs.k[:])
	cipher.Seal(encNothing[:0], zeroNonce[:], nil, hs.hash[:])

	// update hash
	crypt.Hash(&hs.hash, encNothing[:])
...
func (hs *Handshake) Receive(rTPub ppk.PublicKey,
	iSec ppk.PrivateKey, encNothing [sizeEncNth]byte) error {
	// lock handshake
	hs.Lock()
	defer hs.Unlock()
	...
	// update chaining key
	hkdf.HKDF(hs.c[:], rTPub[:], &hs.c)

	// update hash
	crypt.Hash(&hs.hash, hs.hash[:], rTPub[:])

	// update chaining key from shared secrets
	var ss [ppk.KeySize]byte
	ss = hs.tempKeys.secret.SharedSecret(rTPub)
	hkdf.HKDF(hs.c[:], ss[:], &hs.c)
	ss = iSec.SharedSecret(rTPub)
	hkdf.HKDF(hs.c[:], ss[:], &hs.c)
	...
	cipher, _ := chacha.New(hs.k[:])
	_, err := cipher.Open(nil, zeroNonce[:], encNothing[:], hs.hash[:])
	if err != nil {
		return err
	}
